package com.dev.lishaboramobile.Global.Data.Operations;
...
public interface TradersDao {
    ...
    LiveData<TraderModel> getAllByArchivedStatus(int archived);

    @Query("SELECT * FROM TRADERS WHERE deleted =:deleted")
    LiveData<TraderModel> getAllByDeleteStatus(int deleted);

    @Query("SELECT * FROM TRADERS WHERE dummy =:dummy")
    LiveData<TraderModel> getAllByDummyStatus(int dummy);

    @Query("SELECT * FROM TRADERS WHERE status =:status")
    LiveData<TraderModel> getAllByStatus(String status);



    @Query("SELECT * FROM TRADERS WHERE code =:code")
    LiveData<TraderModel> getTradersByCode(String code);

    @Query("SELECT * FROM TRADERS WHERE names =:names")
    LiveData<TraderModel> getTraderByNames(String names);

    @Query("SELECT * FROM TRADERS WHERE entitycode =:entitycode")
    LiveData<TraderModel> getTradersByEntityCode(String entitycode);
